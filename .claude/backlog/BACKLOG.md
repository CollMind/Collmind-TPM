...
| [[T-257]] | ✅ **KARAR `(c)`** — genel `POST /approvals/:id/{approve,reject}` **kaldırılır** (`0` tüketici + `K-2.5.6` atomiklik ihlali; `409` yazmak kullanılmayan bir ucu korumak olurdu). `GET` uçları KALIR. ⚠️ `cancel` aynı turda **ölçülecek** · `T-256`'nın `K-2.5.11` pini öksüz kalmamalı | **P1** | backend-engineer | todo |
