...
| [[T-290]] | ⚠️ **İKİ AYRI ÖZNE, AYNI SINIF** — `app-runtime-grants` self-test case 15 **ve** `settlement.e2e-spec.ts`: ikisi de *izole yeşil, tam sette aralıklı kırmızı*. ⇒ Aranan şey **özne değil ORTAK MEKANİZMA** (çevre: tam ↔ izole koşum). `app-runtime-grants` case 15 **ARALIKLI** düşüyor — runner altında `1/3`, tek başına `2/2` yeşil. Geçici dizin çakışması değil (`mktemp -d`). ⚠️ Aralıklı bir self-test, guard'ı aralıklı olarak **güvenilmez** yapar | P2 | debugger | todo |
