...
| [[T-241]] | `POST /users` rol + kapsam **birlikte**; kapsamsız kullanıcı yaratılamaz. Atomiklik **DB seviyesinde** kanıtlı. ⛔ Review `B1` (fail-open joker) kapandı `25f6b07`, `B2` → [[T-243]]. `code-reviewer` ikinci tur bekliyor | **P1** | backend-engineer | review |
| [[T-243]] | 🔴 **`T-241` arayüzü KIRDI** — `UserForm`'un varsayılan rolü `PLANNER` ve frontend `CreateUserDto`'da `scope` YOK → **varsayılan yol `400`**. Kayıt `Z12` + `EK_E` (`⚠️`/`🔒`). Brief `B1` kararıyla yazıldı | **P1** | frontend-engineer | todo |
...
| [[T-242]] | Kapsam **GÜNCELLEME** yolu — `T-241`'den ayrıldı (*yaratma bayrağı açar, güncelleme yönetim yeteneği*). ⛔ **`P2` AMA `T-235 ADIM 3`'ü BLOKLUYOR**: dal kalkınca terfi eden kullanıcı eski dar kapsamıyla `ADMIN` olur, kusur **sessiz** | P2 | architect | todo |
