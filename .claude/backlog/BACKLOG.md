...
| [[T-276]] | 🔴 **SAHTE SELF** — `GET /approvals/pending` `userId`'yi alıyor, taşıyor ve **`WHERE`'e koymuyor**. Ölçüldü: FINANCE, ADMIN'in kaydını görüyor; poz.kontrol `/my-requests` **`0`** (yüklemi olan kardeş, aynı fixture). ⛔ **DUR** — doğru yüklem (*"bana atanmış"* mı *"benim açtığım"* mı) bir **ürün kararı**, `approval_levels` `0` satır olduğu için koddan okunamıyor | **P0** | architect | todo |
