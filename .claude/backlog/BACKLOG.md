...
| [[T-238]] | ✅ Karar **`DROP`** — `user_scopes.channel_id` (migration `1809`). `K-2.1.4` kanalı `cpl_id`'den garantili türetiyor; kolon ikinci ve zorlanmayan kaynak. Ölçüldü: okuyan `0` · yazan `0` · dolu satır `0/37`. ⚡ **`T-235`'in bayrağı buna bağlı** | **P1** | data-engineer | todo |
