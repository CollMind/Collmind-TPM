...
- [[T-375]] ⛔ Zarflarda `category` **kolonda BOŞ** (dördünde de; `channel` yalnız `metadata`'da) — `T-373` düzeltmesi sonrası on-invoice satırları zarf **bulamayacak** · iki seçenek: **(A)** veri düzeltmesi ↔ **(B)** `K-2.2.2`'ye kategori muafiyeti (**yeni BRD kuralı**) — **ürün sahibi kararı** — **todo** `P0`
