...
- [[T-387]] ⛔ `Z107` son review artıkları — `23P01` sınıfının **ÜÇÜNCÜ üreticisi** (`splitEnvelope`'ın `updateEnvelope`'ı) sarmalanmamış ⇒ `500` · DTO'ya göre **geçerli** bir gövde de `500` üretiyor (`period` format doğrulaması **yok**) · ⛔ **`manager?` opsiyonel: unutulan çağrı yeri SESSİZCE eski davranışa düşer** — tip kapısı mı disiplin mi, **karara bağlanacak** — **todo** `P1`
...
- [[T-385]] ⛔ `Z107` demo hizalaması **2026-09-30'da SONA ERİYOR** — `isoToday()` kullanan **beş** dosya + `A13`/`A13b` Q3 zarfına bağımlı hâle geldi ⇒ **1 Ekim'de altı suite kendiliğinden kırmızı**, sebebi **bu commit** · `T-329`/`T-333`'ün **TERSİ** (veri sabit, *"bugün"* hareketli) — **todo** `P1`
