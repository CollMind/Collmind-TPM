...
| [[T-289]] | ⛔ **HÜKÜM: UÇ KALDIRILIR** (ürün sahibi 2026-08-26 — *"rol kararı değil, `K-2.2.4`'ün savunması"*). `POST /budget/reserve` — doğrulanmamış `agreementId` ile POSTED defter satırı. Dört yüzeyde ölçüldü: ekranda `requiredRole` YOK (poz.kontrol 40 rotada var) · alan **serbest metin** · servis anlaşmayı **doğrulamıyor** · `AccessScope` atfı **0** (poz.kontrol 7 serviste var). `K-2.2.4` gerilimi: onay tetikleyicisini ATLAYAN ikinci yol | **P1** | backend-engineer | todo |
