...
- [[T-378]] ⛔ Anlaşma kategorisi **SAKLANDI** — `T-028e`'nin *"DO NOT backfill … goes stale"* yasağı `1828` ile ihlal edildi · hüküm kazanır ama **kopya olduğu için değil, artık kopya OLMADIĞI için** (beyan ↔ katalog çapraz-doğrulaması) · FU yeniden sınıflandırılırsa **bayatlar** — **todo** `P1`
- [[T-379]] ⛔ `GET /budget/status` — frontend **UUID**, backend **KOD** bekliyor (`categoryId` adı **tipi** hakkında yanlış söylüyor) · ve zarf çözümlenemezse uç sessizce **`0/GREEN`** diyor · **bu dalga getirmedi, ÖRTÜSÜNÜ KALDIRDI** — **todo** `P1`
- [[T-380]] ⛔ `period LIKE 'YYYY%'` + kanal-wildcard ⇒ aynı kategoride iki zarf **ikisi de** eşleşir, seçim `createdAt DESC`'e düşer = `K-2.2.3a`'nın yasakladığı **gizli tie-break** · bugün ateşlenemez (`T-273` körlüğü) — **todo** `P1`
...
- [[T-383]] ⛔ `"bugün"` **ÜÇ FARKLI TABANDA** yaşıyor — **sunucu-yerel · UTC · DB OTURUMU** · üç ayrı **ŞEKİL**, yedi yer; ikisi **KALICI KİMLİK** yazıyor (`PLAN-<yıl>-Q<çeyrek>` · `<tip>-<yıl>`) · `ADIM 3` **birini** kapattı · ⛔ *bir liste vermek, EVRENİ tanımlamak değildir* — **todo** `P1`
...
- [[T-377]] ⛔ `cleanup-data.ts` `ledger_entries`'i **hiç** silmiyor ⇒ `seed:cleanup-and-seed` **RESTRICT**'e çarpar · bugüne kadar patlamadı çünkü tablo **boştu** — `T-273`'ün zaman eksenli üyesi: **verinin yokluğu örttü** — **todo** `P1`
