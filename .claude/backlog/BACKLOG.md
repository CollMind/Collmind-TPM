...
| [[T-282]] | **`CLAUDE.md` envanteri** — desen anlatan kaç kural **mevcut bir aracı** adlandırmıyor? `Z34`'ün genel formu: araç vardı, kural işaret etmiyordu. Refleks-açığı sınıfını **kökten** kapatır. ⚠️ Aciliyeti yok ⚠️ **DÖRT VAKALIK** (2026-08-24): yorum kirliliği ×4 · ham-grep ×1 · dar tarama ×3 — araçlaşma eşiği (üç vaka) **geçildi**. Öncelik yükseltme kararı ürün sahibinde | P3 | architect | todo |
| [[T-283]] | **`211` satırlık rota→hücre tablosu REPODA YOK** — `B3B1_DEVIR_BRIEF §2` *"üç listeyi repodan yeniden türet"* diyor ve *"atlanamaz"* ekliyor, ama eşleme `B3A:307`'ye göre **ajanın raporunda**. Ölçüldü: dizinden türetilemiyor (`SHARED_* 39` ↔ `shared/ 42`). ⛔ **`B3b-1`'in ÖN KOŞULU** | P1 | architect | todo |
...
| [[T-289]] | ✅ **KALDIRILDI** (`K6c/d`) — ⚠️ ve açılış iddiası (*"`PLANNER` uydurma id ile POSTED satır üretebilir"*) **ÇÜRÜDÜ**: repro her seferinde `500` verdi, uç **yapısal olarak tamamlanamıyordu**. Karar üç bağımsız sebeple ayakta kaldı. **HÜKÜM: UÇ KALDIRILIR** (ürün sahibi 2026-08-26 — *"rol kararı değil, `K-2.2.4`'ün savunması"*). `POST /budget/reserve` — doğrulanmamış `agreementId` ile POSTED defter satırı. Dört yüzeyde ölçüldü: ekranda `requiredRole` YOK (poz.kontrol 40 rotada var) · alan **serbest metin** · servis anlaşmayı **doğrulamıyor** · `AccessScope` atfı **0** (poz.kontrol 7 serviste var). `K-2.2.4` gerilimi: onay tetikleyicisini ATLAYAN ikinci yol | **P1** | backend-engineer | todo |
...
| [[T-296]] | ⛔ **İKİ CANLI `400` daha** — `spend-trend?granularity` · `variance-analysis?comparisonType`, `T-294` ile **aynı sınıf ve aynı dosya**, ve frontend ikisini de **her istekte** gönderiyor. Sınıfı `months`'unkinden **dar** (sessiz yanlış değer yok, yalnız `400`). ⚠️ `§7.1` sayımı brief'te **istenmemişti** — Team Lead açığı. Çözüm deseni repoda **zaten var** (`BudgetVarianceQueryDto`) | **P1** | backend-engineer | todo |
| [[T-297]] | **`GET /users` göçünün ÖNÜ AÇILDI** — `B3 W3`'ün dışarıda bırakma gerekçesi (*"göçürmek `FINANCE`'ı düşürürdü"*) `K1`'in `FINANCE`'ı düşürmesiyle **boşta kaldı**. Göç artık **davranış-koruyucu**: `USER_MANAGE = {ADMIN}` ↔ rota `{ADMIN}` birebir. `USER_MANAGE` hücresinde `@Roles` taşıyan **tek** rota | P2 | backend-engineer | todo |
...
| [[T-299]] | **`MODES_READ` hücresi YEDİ ayrı rol kümesi taşıyor** — hücre-normalizasyonu açık. `K2` `ledger` **ailesini** `6/6` yaptı, **hücreyi** değil. `SUMMARY`/`MODES_READ` karar paketiyle birlikte çözülür | P2 | architect | todo |
...
| [[T-306]] | ⛔ **CANLI `500`** — `finance-reporting/budget-variance`, `column envelope.cplid does not exist`. RBAC'la ilgisiz, **önceden var**; `B4` düğme pini guard'ı açınca **anında** göründü. ⚡ Örten şey bir kusur değil, **çalışan bir yetki kontrolüydü**. `T-294`/`T-296` ailesinin **üçüncü** üyesi, **aynı serviste**. ⚠️ `Z43`'ün `+CM ×3` genişlemesi tam bu servise bakıyor — genişleme bu task kapanmadan **inmez** | **P1** | debugger | todo |
| [[T-307]] | ⛔ **CANLI CROSS-TENANT** — bir kiracının `ADMIN`'i başka kiracıyı **okuyabilir/değiştirebilir/SİLEBİLİR** (`/tenants` 8 rota, `tenant.service`'te `tenantId` **0** atıf; poz.kontrol: `customer.service` **37**). ⚡ `main.tenants=1` olduğu için görünmüyor — *"verinin yokluğu örter"*in **ders kitabı vakası**. ⛔ `RLS` bunu **çözemez**: `tenants` tablosu `tenant_id` **taşımıyor** | **P1** | backend-engineer | todo |
| [[T-308]] | ⛔ **`v_budget_summary` `security_invoker` YOK** — her `RLS` politikasını **ATLAR** ve `guards` **yeşil kalır**. Probe: invoker'sız → **iki tenant**, invoker'lı → **tek**. **Alan A** yüzeyinde, canlı. ⇒ `RLS` zincirinin **BİRİNCİ** adımı + `reloptions` kapısı | **P1** | data-engineer | todo |
