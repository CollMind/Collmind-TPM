...
| [[T-266]] | **KAPSAM RATCHET'i** — ⛔ `B2`'den ÖNCE kurulmalı (`Z19b`): sonradan alınan baseline **düşen uçları kapsamlı sanır**. Guard *"kapsam var mı"* ölçMEZ (ölçemez — `T-079`) — **listenin büyümediğini** ölçer. Mekanizma: `A∪B∪C` + sınıflandırılmamış → `exit 2` | **P1** | qa-engineer | todo |
