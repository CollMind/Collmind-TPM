...
| [[T-255]] | 🔴🔴 `GET /users/:id` — herhangi bir kimliklenmiş kullanıcı (`READONLY` dâhil) **başkasının TAM kaydını** okuyor: `passwordHash` · `refreshToken` · `passwordResetToken`. Üç ayrı kusur (rol · sahiplik · ham entity). Davranışsal, iki yönde doğrulandı | **P1** | backend-engineer | todo |
