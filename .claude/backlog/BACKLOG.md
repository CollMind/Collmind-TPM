...
- [[T-375]] ⛔ Zarflarda `category` **kolonda BOŞ** (dördünde de; `channel` yalnız `metadata`'da) — `T-373` düzeltmesi sonrası on-invoice satırları zarf **bulamayacak** · iki seçenek: **(A)** veri düzeltmesi ↔ **(B)** `K-2.2.2`'ye kategori muafiyeti (**yeni BRD kuralı**) — ⭐ **HÜKÜM `(A)`, GENİŞLETİLMİŞ** (2026-09-07, `Z102`): zarf **MODELİ** — kategori+dönem ZORUNLU, kanal **tanımlı-wildcard**, TEK kaskad (`K-2.2.1'`/`K-2.2.3a`), **anlaşma = TEK kategori** (`K-2.2.3b`), *"bugün"* = tenant-TZ · ⭐ **İNDİ** (`Z103`): kaskad + 8 zarf Σ2.3M + anlaşma=tek kategori + `PATCH` çapraz-doğrulaması · `tsc 0 · guards 0 · unit 1538 · e2e 877 · T-047 PASS` · review **iki tur**, ilki `PUSH'A UYGUN DEĞİL` (`B1`) · **KAPSANMADI:** `ADIM 3` (tenant-TZ) · `S2` · `1830` — **in-progress** `P0`
