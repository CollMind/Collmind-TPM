...
| [[T-289]] | ⛔ **`POST /budget/reserve` — doğrulanmamış `agreementId` ile POSTED defter satırı.** Dört yüzeyde ölçüldü: ekranda `requiredRole` YOK (poz.kontrol 40 rotada var) · alan **serbest metin** · servis anlaşmayı **doğrulamıyor** · `AccessScope` atfı **0** (poz.kontrol 7 serviste var). `K-2.2.4` gerilimi: onay tetikleyicisini ATLAYAN ikinci yol | **P1** | backend-engineer | todo |
| [[T-290]] | `app-runtime-grants` self-test case 15 **ARALIKLI** düşüyor — runner altında `1/3`, tek başına `2/2` yeşil. Geçici dizin çakışması değil (`mktemp -d`). ⚠️ Aralıklı bir self-test, guard'ı aralıklı olarak **güvenilmez** yapar | P2 | debugger | todo |
