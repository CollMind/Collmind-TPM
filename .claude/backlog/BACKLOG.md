...
| [[T-294]] | ⛔ **`cash-flow-projection` — `400` bir SESSİZ YANLIŞ DEĞERİ ÖRTÜYOR.** `months` DTO'da yok ⇒ whitelist reddi; ama kabul edilseydi `ParseIntPipe`'sız string → `getMonth()+"12"` → **yıl 2085**. ⇒ Whitelist düzeltmesi **tek başına inmez**. Ayrıca parametresiz `500`'ün sebebi **ölçülmedi** — üçüncü kusur olabilir. `K3`'ün koruduğu widget sunucuda kırık ⇒ dalganın **kapanış tanımına** dokunuyor | **P1** | backend-engineer | todo |
