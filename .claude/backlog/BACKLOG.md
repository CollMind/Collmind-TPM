...
| [[T-271]] | 🔴 **LTA'nın DÖRT canlı ADMIN ucu bugün ÇALIŞMIYOR** — `app_runtime`'ın `lta_agreements`/`lta_rates` üzerinde **sıfır INSERT/UPDATE** (doğrulandı: `INSERT=false`) + `findOverlappingAgreements`'in ham SQL'inde **tipsiz parametre** (`$5`, GRANT'ten bağımsız, hep düşüyor). ⛔ Biri diğerini **maskeliyor**: `create`/`activate` → Kusur 4 (INSERT'e hiç ulaşmıyor), `update`/`terminate` → Kusur 3. Aynı turda gitmeli | **P0** | backend-engineer | todo |
