...
| [[T-255]] | 🔴🔴 **`P0`** `GET /users/:id` — herhangi bir kimliklenmiş kullanıcı (`READONLY` dâhil) **başkasının TAM kaydını** okuyor: `passwordHash` · `refreshToken` · `passwordResetToken`. Üç ayrı kusur (rol · sahiplik · ham entity). Davranışsal, iki yönde doğrulandı | **P1** | backend-engineer | todo |
...
| [[T-257]] | 🔴 Genel `POST /approvals/:id/{approve,reject}` domain akışını **atlıyor** → `approval_requests` `APPROVED`, plan `PENDING_APPROVAL`, bütçe **yazılmamış** = `K-2.5.6` ihlali. ✅ Karar `(b)` 409; ⚡ ölçüm `(c)`'yi masada tutuyor: **sıfır tüketici** | **P1** | backend-engineer | todo |
| [[T-258]] | 🔴🔴 **`P0`** `GET /tenants/:id` — `READONLY` bile **9 kullanıcının tamamını** `passwordHash`+`refreshToken` ile alıyor (tek istekte). `@Roles` yok · `relations:['users']` ham · **tenant-scope predicate'i YOK** (cross-tenant riski, tek tenant olduğu için bugün gösterilemiyor) | **P1** | backend-engineer | todo |
