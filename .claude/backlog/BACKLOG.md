...
| [[T-238]] | `user_scopes.channel_id` **DÜŞÜRÜLDÜ** (`1809`). Üç dal da ampirik: DROP · **NO-OP** · **İPTAL** (sahte dolu satır → throw, kolon bozulmadı). ⛔ `code-reviewer` bekliyor → sonra **bayrak** | **P1** | data-engineer | review |
| [[T-241]] | 🔴 **`user_scopes`'a YAZMA YOLU YOK** — HTTP ucu `0`, `POST /users` kapsam yazmıyor; tek yollar seed + tek-seferlik backfill. Migration `1792` *"admin manuel eklemeli"* diyor, **o yol da yok**. ⚡ Bayrağı BLOKLUYOR | **P1** | architect | todo |
