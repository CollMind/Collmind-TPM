...
## ⭐ `v2` — 16 AİLE, 216 KURAL (`ADIM-4`, 2026-09-09)

> ⛔ **Bu belge `ADIM-4` oturumunda YENİDEN YAPILANDIRILDI.** Hiçbir kural silinmedi;
> **üçü** `docs/process/GOZLEM_GUNLUGU.md`'ye taşındı (başlık + özet burada KALDI,
> karşılıklı atıflı — `F12`/`0006-R` deseni). **İçerik değişmedi.**

```
AİLE = AYNI TETİKLEYİCİ-SORUNUN sorulduğu maddeler.   ⛔ BOYUT DEĞİL.
```

⛔ Her ailenin başında bir **🔎 tetikleyici** var: ajan kuralın **adını** bilmez,
**durumu** bilir. Tetikleyici bir başlık değil, bir **arama yüzeyidir**.

⚠️ **`133` maddenin TÜREVİ ÖLÇÜLMEDİ** — `R1` yalnız *vakasız* maddede türev ister;
bu küme *"tek işaret"* proxy'siyle ayrıldı ve proxy **kabadır**. **Korumacı varsayımla
KALIYORLAR**: yanlışsa fazladan madde kalır, hiçbiri yanlışlıkla günlüğe inmez.
⇒ Ölçüm **olay-tetikli**: bir madde arandığında **bulunamıyorsa**, ya da `Z110`
metriğinde **tekrar-oranı düşmüyorsa**, o madde ölçülür.


---

# AİLE F00 — META — kural kendi yazarına uygulanır

> ## 🔎 ne zaman: *"bir KURAL / KAPI / ARAÇ yazıyorum — kendime uyguluyor muyum?"*
...
## YARGI SORUSUNA **MEKANİK PROXY** YAZILMAZ (ZORUNLU)

**Mekanik bir tarama bir ADAY LİSTESİ üretir. SINIFLANDIRMA bir İNSAN YARGISIDIR.**
⛔ Bir proxy'nin çıktısı **hiçbir zaman `[ÖLÇÜLDÜ]`/teşhis etiketiyle raporlanmaz.**

**Üç ölçülmüş vaka, hepsi `ADIM-4` (2026-09-08/09):**
```
1  "kaç vaka"      ← tarih + task proxy'si        → ALTIDA DÖRT yanlış
2  "türev sayısı"  ← başlıktan otomatik terim     → dağılım ÇÖKTÜ (203/218 "≥10")
3  "aile"          ← anahtar-kelime eşleştirme    → 125/218 GRİ (desenin ARTIĞI)
```
> ### ⛔ VE DÖRDÜNCÜ VAKA SINIFI GENİŞLETTİ: anahtarı **ELLE** seçmek de yetmedi.
> ### `Z83` maddesinin türevi başlık-anahtarıyla **1**, kanonik atıf adıyla **81** çıktı.
> ### ⇒ Çözüm *"insan yargısı"* değil, ***DOĞRU DİLDE*** insan yargısıdır.

📌 Kurtaran kural aynı ailedeydi: *"Arama terimi, **ARANAN YERİN DİLİYLE** seçilir"*
(`F02`). ⛔ Bir yargı sorusunu ölçerken **anahtar, ARANAN YERİN atıf dilinden** seçilir —
başlığın sözcüklerinden değil.

## GÖZLEM → **İKİNCİ** VAKADA KURAL → **ÜÇÜNCÜ**DE ARAÇ (ZORUNLU)

```
1. vaka   GÖZLEM   — günlüğe (docs/process/GOZLEM_GUNLUGU.md)
2. vaka   KURAL    — DISIPLIN'e, ailesi ve 🔎 tetikleyicisiyle
3. vaka   ARAÇ     — kuralı HATIRLAMAK yerine ÇAĞIRILAN bir şey
```
⛔ **Ve eşik AŞILDIYSA borç birikir:** `T-128` (mutasyon aracı) `2026-08-10`'da açıldı,
`2026-09-08`'de indi — arada **altı vaka**. Kural *"üçüncüde araç"* diyordu.

> ### ⛔ Bir kuralın **işe yaraması**, aracın ihtiyacını **ortadan kaldırmaz** —
> ### her vakada *"kural yakaladı"* diye kapanır ve borç **görünmez** büyür.

📌 Bugüne kadar bu eşiği geçen araçlar: `mutate.sh` (6 vaka) · `scan.sh` (5 vaka) ·
`migration-verify.sh` (elle koşulan liste). ⛔ Ve *"sayı yazma / elle üye-sayısı"*
çifti **19 vakayla** `scan.sh`'a bağlandı.
...
# AİLE F03 — PİN ve KÖR NOKTA

> ## 🔎 ne zaman: *"bu testi/pini neden YEŞİL sanıyorum?"*
...
⛔ **GÖVDE `docs/process/GOZLEM_GUNLUGU.md`'YE TAŞINDI** (`ADIM-4`, türev **0**).
**Özet:** bir pinin körlüğü *"pin kör"* diye tek parça söylenemez — **mutasyonun
TÜRÜNE göre** ayrışır (dekoratör düşmesi ⇒ **kör** · üyelik daraltması ⇒ **görüyor**).
⇒ ⛔ **İkinci vakasında GERİ TERFİ EDER.**
...
# AİLE F04 — KAPI ve DOĞUM

> ## 🔎 ne zaman: *"bir KAPI/GUARD yazıyorum ya da koşuyorum"*
...
# AİLE F05 — GEREKÇENİN YAŞAM DÖNGÜSÜ

> ## 🔎 ne zaman: *"bu GEREKÇE hâlâ geçerli mi?"*
...
# AİLE F07 — TANIM → YAZAR → KISIT

> ## 🔎 ne zaman: *"bir KOLON / KISIT / YETKİ yüzeyine dokunuyorum"*
...
# AİLE F08 — SESSİZ VARSAYILAN

> ## 🔎 ne zaman: *"bir DEĞER YOKSA ne oluyor?"*
...
⛔ **GÖVDE `docs/process/GOZLEM_GUNLUGU.md`'YE TAŞINDI** (`ADIM-4`, türev **0**).
**Özet:** bir araç hangi hedefe/DB'ye/zincire baktığını **çıktısında söylemiyorsa**,
o çıktı bir **kanıt değildir**. Kardeşi: **yüksek sesle eksik kalmak**.
⇒ ⛔ **İkinci vakasında GERİ TERFİ EDER.**
...
# AİLE F09 — PARALEL ŞERİT ve DEVİR

> ## 🔎 ne zaman: *"bir ŞERİT açıyorum ya da DEVREDİYORUM"*
...
# AİLE F10 — BRIEF · BELGE · KAYIT

> ## 🔎 ne zaman: *"bir BRIEF / RAPOR / KAYIT yazıyorum"*
...
# AİLE F11 — SEED ↔ FIXTURE

> ## 🔎 ne zaman: *"TEST VERİSİ ya da TARİH/DÖNEM kuruyorum"*
...
⛔ **GÖVDE `docs/process/GOZLEM_GUNLUGU.md`'YE TAŞINDI** (`ADIM-4`, türev **0**).
**Özet:** bir brief'in okuma listesindeki her **yol**, bir `[ÖLÇÜLDÜ]` iddiasıdır ve
dosyalar **taşınır** ⇒ yazarken `ls` ile doğrulanır.
⇒ ⛔ **İkinci vakasında GERİ TERFİ EDER.**
...
> ## 🔎 ne zaman: *"bir şeyi TAŞIYORUM — ne KAYBOLUR?"*
...
# AİLE F14 — ORTAM ve BAYATLIK

> ## 🔎 ne zaman: *"ÖLÇÜM ORTAMIM temiz mi?"*
