...
| **E15** | **`src/common/numeric/` Alan A'dadır, doğuştan bloklayıcıdır, ve `money-float` dedektörü orada ateşlemez** | E10'un davranış testi doğrudan uygulanır: `mechanic-input.ts` bir TRY toplamını bir kurala karşı **karşılaştırır** (üçüncü koşul), ve C2a/C2b/C3'ten sonra bu dizin para semantiğinin **tek türetim noktasıdır**. Her şeyin aktığı dosyanın ratchet'in beyan edilmiş kapsamı dışında olması, ratchet'in tanımı gereği kusurudur. Modül F1'de doğduğu, markalı tiplerle yazıldığı ve float taşımadığı için E11 uyarınca **REPORT_ONLY değil, doğuştan bloklayıcıdır**: `NEW_MODULES`'e ve Alan A listesine girer, tabanı **0**'dır ve 0 kalmalıdır. **Ama bu dizin kesinlik primitiflerinin yaşadığı yerdir:** para ayrıştırma, ölçek dönüşümü ve kanonik yuvarlama, tanımı gereği `Number(` ve `Math.round` kullanır — kesinliği *kaybettirmek* için değil, *üretmek* için. `money.ts:81`'in rakam alt-string'inde `Number()` çağırması tam olarak kesin tamsayı elde etmenin yoludur; `rounding.ts:47`'nin `Math.round`'u yasaklanacak bir çağrı değil, yasağın her şeyi içine akıttığı **tek yaptırımlı yuvarlama primitifidir**. Metinsel tarayıcı bu ayrımı yapamaz ve yapması beklenmez — semantik bilmiyor. Bu nedenle dedektör bu dizini **tanır ve ateşlemez** (`money-float.sh` `EXACTNESS_PRIMITIVES`). Bu bir **allowlist istisnası değildir**; guard'ın kendi bilgisidir — `migration-schema`'nın `'main.x'::regclass`'ı şema-güvenli olarak tanıması ile aynı sınıf. `allowlist.txt`'in kendi kuralı bunu zorunlu kılar: *"bilinen bir yanlış pozitif deseni guard'ın kendisi tanımalıdır."* **Koruma nereden geliyor:** dedektör susturulduğu için koruma başka yerden gelmek zorundadır — bu modülün kesinlik garantisi F1'in **17 property-based testi** (`numeric.property.spec.ts`) tarafından tutulur, bu tarama tarafından değil. E15 "muaf tuttuk, koruma yok" değil, **"daha güçlü bir enstrümanla korunuyor"** demektir. **Dizine dosya eklemenin kuralı:** istisna dizin bazlıdır, dolayısıyla sonradan eklenen her dosya da muaf olur. Bu nedenle bu dizine bir dosya koymak, **o dosyanın bir kesinlik primitifi olduğunu ve property-based testle korunduğunu iddia etmek** anlamına gelir. **İş mantığı bu dizine konmaz.** Bu tanıma uymayan dosya kendi modülünde yaşar, orada dedektör ateşler. **E16 ile güncellendi:** muafiyet artık dizin bazlı değil, **dosya bazlı** — aşağıdaki satıra bak. Bu maddedeki "dizine dosya koymak iddia etmektir" kuralı o zaman yalnız yorumda yaşıyordu; E16 onu guard'a bağladı. **Ölçüm notu:** ilk uygulama sessizce hiçbir şey filtrelemedi — BSD `grep` BRE'de `\?` desteklemiyor, desen hiç eşleşmedi. Filtre uygulanmış görünüyordu. Yalnızca taban **ölçüldüğü** için yakalandı; "ekledim, çalışıyordur" denseydi guard sessizce kör kalacaktı. Şimdi `money-float.sh` içinde bir **self-test** her koşumda iki yönü de sabitliyor (primitifler dışlanıyor mu · normal Alan A dosyası hâlâ taranıyor mu), mutasyonla kanıtlandı: desen bozulunca exit 2. Bu, CLAUDE.md §2.6/§2.7'nin **doğrulama maskeleme** ailesinin beşinci üyesidir — bu sefer boru hattı değil, **eşleşmeyen desen**. |
| **E16** | **E15 muafiyeti DİZİN bazlı değil, DOSYA bazlı olur ve guard'a bağlanır** | E15 `src/common/numeric/`'i dizin olarak muaf tuttu ve bir kural yazdı: *"bu dizine dosya koymak, o dosyanın bir kesinlik primitifi olduğunu ve property-based testle korunduğunu **iddia etmektir**; iş mantığı buraya konmaz."* **O kural yalnız bir JSDoc yorumunda yaşıyordu** — dizine sonradan düşen her dosya, kimse böyle bir iddiada bulunmamış olsa da otomatik muaf oluyordu. **Somut istismar yolu ölçüldü** ([[T-087]] yazılırken): `plan.service.ts:119` (`Number(raw)`) o dosyanın 36 ratchet bulgusundan biri; fonksiyonu muaf dizine taşımak dosyayı **35**'e düşürür ve bulgu hedefte **yeniden görünmez** — hiçbir borç ödenmeden ratchet bir ödeme kaydeder. Dizin bazlı bir kural, **ödemeyi taşımadan ayırt edemez**, ve bir kez yapılırsa kalıp olur. **Yeni şekil:** muaf dosyalar `scripts/guards/exactness-primitives.txt`'te tek tek beyan edilir; filtre `grep -Fxv` ile **tam satır** eşleştirir (prefix değil — prefix, beyan edilen yolun altındaki her şeyi muaf tutup E15'e geri döndürürdü). Dizine bildirilmemiş bir dosya **taranır**, ve ratchet'in *"NEW Domain A file"* kontrolü onu yakalar. Böylece E15'in JSDoc kuralı **insan disiplininden mekanizmaya** taşınır — `SYSTEM_INVARIANTS.md`'nin kuruluş fikri: guard'ı olmayan invariant temennidir. **Ölçümle kanıtlandı, iki yönde:** bildirilmemiş bir dosya dizine konunca tarandı ve ratchet exit 1 verdi (*NEW Domain A file with 1 findings*); bildirime eklenince muaf oldu ve exit 0'a döndü. **Ve self-test'in kendisinde bir kusur bulundu:** ilk hâli filtreyi **kendi kopyasıyla** kuruyordu, taramanın kullandığıyla değil. Mutasyon koşumu bunun bedelini gösterdi — taramanın filtresini prefix eşleşmesine (yani tam olarak E16'nın engellediği regresyona) çevirmek self-test'i **yeşil bıraktı ve guard exit 0 verdi**. Filtre tek bir `apply_primitive_filter` fonksiyonuna indirildi; tarama ve self-test aynı yerden geçiyor. Sonrasında üç mutasyon (filtreyi no-op yap · prefix'e çevir · bildirimi boşalt) **üçü de exit 2** ile yakalandı. Ders §7 ailesinden: **bir testin kendi kopyasını sınaması, test etmemekle aynı kapıya çıkar.** |
| **E17** | **Alan A üyelik testi frontend'e OLDUĞU GİBİ geçmez — uyarlanır** | E10'un bağlayıcı testi (*"bir modül para üretiyor, para kalıcılaştırıyor veya parayı bir eşikle karşılaştırıyorsa Alan A'dadır"*) bir DB yazması varsayar. **Frontend para üretmez ve kalıcılaştırmaz** — ayrıştırır, biçimlendirir ve toplar. Test olduğu gibi taşınırsa frontend'in tamamı Alan A dışında kalır, ve [[T-106]]'nın ölçtüğü şey tam olarak budur: `replace(/,/g,'') + parseFloat` para yollarında yıllarca durdu, **hiçbir kapı saymadı**. **Frontend üyelik testi:** bir dosya (i) kullanıcının yazdığı/yüklediği bir para değerini **ayrıştırıyorsa**, (ii) para alanlarını istemcide **topluyorsa**, ya da (iii) bir para alanını bir eşikle **karşılaştırıyorsa** Alan A'dadır. Testin amacı korunur, mekanizması değişir — E10'un kendi uyarısı gereği: liste örnekleyicidir, **test kanoniktir**. **Ölçülen bedel (T-111):** taban `collmind.frontend/scripts/guards/money-float-baseline.txt`'in başlığında durur — buraya bir sayı yazmak, bir sonraki kapsam kararında (E18 iki satır aşağıda tam olarak onu yaptı) sessizce bayatlar. **Ve dedektörün şekli önemli:** naif alt-string taraması `BudgetEnvelopeList.tsx`'te 15 bulgu sayıyor, sınır-duyarlı tarama **4** — aradaki 11 `safeNumber(...)` sarmalayıcı çağrısı. Naif sayıya kurulmuş bir taban **borcu değil gürültüyü** ratchet'lar ve düzeltilecek bir şey yokken düzeltme baskısı üretir. |
| **E18** | **Alan A/B sınırının ölçütü verinin KAYNAĞI değil, GİTTİĞİ YERDİR** | Karar 1 Alan B'yi *"analitik — ROI, oran, RAG, raporlama türev değerleri"* diye tanımladı; gerekçesi bu değerlerin **kalıcılaşmaması** ve bir eşikle karşılaştırılmamasıydı, dolayısıyla kayan nokta toleransı kabul edilebilirdi. **Frontend'e taşınırken bu gerekçe bir boşluk açıyor:** `src/utils/export.ts` verisini Domain B uçlarından (`/finance-reporting/*`, `/dashboard/*`) alır — kaynağa bakan bir okuma onu Alan B sayar. Ama dosya **CSV üretir**: kullanıcının indirdiği, Excel'de açtığı, muhtemelen başka bir sisteme yüklediği bir artefakt. Ekranda görünen bir türev değil. **Bağlayıcı ek ölçüt:** bir değer bir **kullanıcı artefaktına** (indirilen dosya, e-posta eki, dışa aktarım) yazılıyorsa, kaynağı ne olursa olsun Alan A'dadır. Gerekçe simetriktir ve bu projede iki ucu da yaşandı: [[T-105]] **kullanıcının hazırladığı** CSV'nin yanlış ayrıştırılmasıydı; ürünün **kendi ürettiği** CSV'nin doğruluğunu Alan B toleransına bırakmak aynı zincirin öteki ucunu açık bırakırdı. **⚠️ Ölçüm, ve dahil etme kararının anlamı (T-111):** `export.ts`'in 16 bulgusunun **hepsi** oran — `toFixed()` üzerinde `utilizationPercent`, `avgRoi`, `gpRoi`, `riskPercentage`, `variancePercent`, `efficiencyScore`. **Para tutarı olan tek bir bulgu yok**; para alanları (`allocated`, `utilized`, `available`, `Amount`) CSV'ye hiçbir float ilkelinden geçmeden yazılıyor. Yani dosyayı Alan A'ya almak bugün **oran borcunu** tabana yazar, para borcunu değil. Bu bir çelişki değil, ratchet'in tanımı: taban mevcut borcu **tutar**, ödemesini istemez; kapı **17.'yi** engeller — ve 17., para alanlarından birine eklenecek bir `parseFloat`/`toFixed` olacaktır. Metinsel tarayıcı oran ile para arasındaki farkı bilemez ve bilmesi beklenmez; bilen şey **üyelik kararıdır**, ve bu errata onu kayda geçirir. |

### Errata ile birlikte verilen kapsam kararları

| # | Konu | Karar |
|---|---|---|
| **A4** | `agreements.mechanic_value` (3/3 NULL, ayırıcısı da 3/3 NULL, tek tüketicisi mock veriyle Alan B'ye gidiyor) | **Dondurulur.** Silinmez, tamamlanmaz, **ölçek kontratına dahil edilmez.** ⚠️ **Gerekçe yenilendi (T-142, ADR 0010):** eski gerekçe *"tamamlanmamış bir yolun kolonu"* idi ve **veri boşluğuna** dayanıyordu. Bağlayıcı BRD onu **tasarım gereği polimorfik** tanımlıyor: `mechanic_value NUMERIC(18,4), -- e.g., 15.00 (TL per unit) or 10.5 (%)` (`docs/brd/01_Main_BRD/Section_04`). Yani kolon *kullanılmıyor* değil, **spec'i var, henüz yazılmadı** — bu ikisi farklı şeydir. Dondurma kararı **geçerli kalır** (ölçek kararı için hâlâ veri yok), ama artık Karar 4'ün (`entered_value` polimorfizmi) **bilinen bir kardeşi** olarak dondurulmuştur; yol yazıldığında Karar 4 ile **aynı bölünme** uygulanır. Kolona yorum bu gerekçeyi taşır. |
...
| v3+E17/E18 | T-111 (frontend ratchet) sonrası: Alan A üyelik testinin frontend uyarlaması (E17) ve **kullanıcı artefaktı** ölçütü (E18) eklendi. Ürün sahibi kararı, 2026-08-09 — `export.ts`'in Alan B sayılması reddedildi. |
