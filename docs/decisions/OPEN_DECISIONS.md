...
| `0064-TENANT` | Çok-kiracılık Phase 1'de **var** mı, yol haritası mı? | `0064 §2` · `§2.5` ↔ `§9.2` | RLS · `0056-K10` · [[T-179]] | **açık** — `§9.2` sayısal hedef veriyor ve NFR bölümünde; `§2.5` *"current: single-tenant"* diyor |
| `0064-SCALE` | Ölçek hedefleri hangi sayı: SKU 5.000 mi 10.000 mi, batch 40-50 mi 500 mü? | `0064 §3` · `§2.5` ↔ `§9.2` ↔ `§1.3`/`§2.1.3` | performans testi hedefleri · [[T-154]] | **açık** — üç beyan, üç sayı, hiçbiri diğerine atıf vermiyor |
| `0063-SSO` | SSO Phase 1'de var mı? | `0063 §2` · `§2.1.4` *"Day 1"* ↔ `§7.7:592` · `§10.1:105` | Phase 1 güvenlik kapsamı | **açık** — 1 tanığa karşı **2**; uzlaştıran okuma adayı: *"SSO-ready"* ≠ entegrasyon |
