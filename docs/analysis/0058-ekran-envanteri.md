...
| frontend commit | `d9bedc5` (`fix(approvals): sayfa içi rol kontrolleri hizalandı — hasRole, ADMIN bypass'ı geri geldi`, 2026-08-11 01:09:11 +03, branch `staging`) — **çalışan ağaç temiz**, meta-repo submodule pointer'ı ile **aynı** (`git submodule status` → ` d9bedc5… (heads/staging)`, öndeki boşluk = senkron). `npm run dev` bu oturumda taze başlatıldı (Vite 5.4.21, port 5173) |
| frontend revizyonu — **sonradan doğrulandı** | Ölçüldü **2026-08-12**, lokal makinede (bu ölçüm turunda kaydedilmemişti; geriye dönük kapatıldı). Doğrulama: `git -C collmind.frontend rev-parse --show-toplevel` → `…/Collmind-TPM/collmind.frontend` (meta-repo kökü **değil** — submodule init edilmemiş bir dizinde bu komut sessizce meta-repo'yu cevaplar, bkz. §2.7 ölçüm maskeleme). `rev-parse HEAD` → `d9bedc5127b421acc6d65efce3eaaea28e9783f2`; `status --short` → **boş**; `remote -v` → `origin https://github.com/CollMind/collmind.frontend.git`. |
| görüntülerin tazeliği | Ölçüm anı 2026-08-11 02:09, HEAD commit'i 01:09 — yani görüntüler **d9bedc5'in bir saat sonrasında** alındı. `git log --since='2026-08-11 02:09' --all` → **boş**: ölçümden bu yana (2026-08-12 itibarıyla) frontend'e **hiç commit gitmedi** ve ağaç temiz. Yani 45 ekran görüntüsü **bugünkü frontend koduna aittir**; ürün özetinin "bugün ne var" bölümü bu revizyona dayanır. |
